...
- **Dampak:** Change detection berjalan lebih sering dari yang diperlukan. Karena state sudah berbasis signal, ini adalah _quick win_ performa yang terlewat.
...
| `features/checkout/checkout.component.ts`      | `resolvedistrictForAddress()`          | GET×N sekuensial         | ❌ Tidak    | Overlay loading saat resolusi alamat                    |
...
_Rekomendasi Modern:_ Untuk call yang bersifat query/data-fetching, gunakan `httpResource()` dari Angular 19.1+ yang menyediakan property reaktif `.isLoading()` dan `.error()` bawaan untuk mereduksi kode boilerplate manual.
...
_\*Catatan Matrix: Skor ditentukan dari perpaduan prioritas bisnis, tingkat keamanan, dan kecepatan pengerjaan (Quick Wins vs Strategic Improvements)._
...
_Quick wins dan perbaikan celah keamanan kritis._
...
_Peningkatan Core Web Vitals dan kelancaran checkout flow._
...
_Peningkatan maintainability kode jangka panjang._
...
_Optimalisasi framework dan aktivasi zoneless._

export const environment = {
  production: true,
  apiBaseUrl: 'https://tokosuburjaya.com:17800/ssonline/api/v1',
  authUrl: 'https://tokosuburjaya.com/connect/token',
  apiUrl: 'https://tokosuburjaya.com:17800/ssonline/api/v1/',
  originCode: '32.71.10.8',
  clientId: '996cc633-23c1-4fb7-a6b6-6fd20dd5051d,',
  clientSecret: '996cc633-23c1-4fb7-a6b6-6fd20dd5051e,',
  scope: 'General-B2C-5Ecd}3+uX3g=%Mxk CRUD-MaBooks-298a66e9 MaBooksScope784a7e2b',
  dokuScriptUrl: 'https://jokul.doku.com/jokul-checkout-js/v1/jokul-checkout-1.0.0.js',
  dokuStyleUrl: 'https://jokul.doku.com/jokul-checkout-js/v1/jokul-checkout-1.0.0.css',

  // Gateway POS (SAP) — dipakai HANYA oleh halaman /admin/pos/*.
  // Sesi & token-nya terpisah dari IdentityServer MalakaBooks.
  posApiUrl: 'http://192.168.1.10:10100',
  posAuthUrl: 'http://192.168.1.10:10100/pos-api/token',
  posOutstandingUrl: 'https://tokosuburjaya.com:17800'
};


...
{
    public record RegisterUserWithFilesCommand(CreateIS4UserWithFilesRequest Request) : IRequest<ValidationResult?>;


    public class RegisterUserWithFilesCommandHandler(IUserRepository userRepository, IProtectedApiClient protectedApi, IUserEntityValidator validator, IFileStorageService fileStorageService) : IRequestHandler<RegisterUserWithFilesCommand, ValidationResult?>
    {
        ...
        public async Task<ValidationResult?> Handle(RegisterUserWithFilesCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                userEntity.Avatar = await _fileStorageService.SaveFileAsync(request.Request.Avatar, "users", cancellationToken);
            }
